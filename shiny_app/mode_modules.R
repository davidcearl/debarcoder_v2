...
library(DT)
library(flowCore)
source('./shiny_app/exp_info_api_module.R')
...
download_fcs_files <- function(cyto_session, 
    ...
    for (fcsFileID in fcsFileIDs) {
        ...
        download_object <- fcs_files.download(cyto_session, expID, fcsFileID)
...
api_mode <- function(input, output, session){
    ...
    cyto_session <- eventReactive(input$submit_connection, {
        ...
        attempt <- authenticate(site=input$domain, 
    ...
    check_if_connected <- function(cyto_session) {
        connection_status <- tryCatch({
            exp.list <- experiments.list(cyto_session, timeout = 1)
        },
        error = function(cond) {
    ...
    fcs_table <- eventReactive(input$experiment_table_rows_selected, {
        ...
        fcs_table <- fcs_files.list(cyto_session(), exp_id)
