...
source('./shiny_app/well_assignment_module.R')
...
assign_wells <- function(bc1, bc2, debarcoded_df) {
    ...
    debarcoded_df$well <- paste0(rep(LETTERS[1:8], each = 12)[rowvals+colvals], str_pad(colvals, 2, pad  = "0")) 
...
split_and_write_fcs <- function(well_assigned_df, gated_flowFrame, comp_matrix, eID,
    ...
    for (well_number in valid.wells){
        ...
        pop <- well_assigned_df[which(well_assigned_df$well==well_number), colnames(exprs(gated_flowFrame))]
        
        pop <- as.matrix(pop)
        colnames(pop) <- fcscolnames
        pop_ff <- flowFrame(pop, description = description(gated_flowFrame), parameters = parameters(gated_flowFrame))
        pop_ff.uncomp <- compensate(pop_ff, solve(comp_matrix))
        description(pop_ff.uncomp) <- description(gated_flowFrame)
        parameters(pop_ff.uncomp) <- parameters(gated_flowFrame)
        pop_ff.uncomp <- clean_ff(pop_ff.uncomp)
        
        suppressWarnings(write.FCS(pop_ff.uncomp, paste0(eID,"/", "/debarcoded/well_",well_number,".fcs")))
    }
}
...
assign_split <- function(input, output, session, setup, fcb, debarcoded){
    ...
    observeEvent(input$writelog, {
        ...
        list.save(sessionlog, "sessionlog.rds")
    })
}
