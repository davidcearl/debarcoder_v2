source('./debarcoder/debarcoder.R')
...
run_debarcoder <- function(input, output, session, fcb_dfs) {
    ...
    output$assign_plot <- renderPlot({
        ...
        yaxis <- strRep(strRep(dbc1()[['channel']], "-", '_'), " ", "_")
        xaxis <- 'SSC_A'
        ggplot(df, aes_string(y = yaxis, x = xaxis)) + 
            scale_y_continuous(trans = logicle_trans()) + 
            geom_point(aes(col = as.factor(df$bc1))) +
            scale_color_discrete(name = "BC1 Levels") +
            ggtitle("BC 1 assignments")
    })
    ...
    output$assign_plot2 <- renderPlot({
        ...
        names(df) <- strRep(strRep(names(df), "-", '_'), " ", "_") #sanitize for ggplot
        yaxis <- strRep(strRep(channel2, "-", '_'), " ", "_")
        xaxis <- strRep(strRep(channel1, "-", '_'), " ", "_")
        
        ggplot(df, aes(color = factor(bc2))) + 
            scale_y_continuous(trans = logicle_trans()) + 
            scale_x_continuous(trans = logicle_trans()) +
            geom_point(aes_string(y = yaxis, x = xaxis), alpha = 0.2, size = 0.5) +
            facet_grid(.~bc1, scales = "free") +
            scale_color_discrete(name = "BC2 Levels")
    })
    return(dbc2)
}