require('pracma')
require('viridis')
source('./debarcoder/gates_transforms.R')
...
compensate_files <- function(comp_matrix, flowframes) {
    ...
    for (frame in names(flowframes)){
        ...
        flowCore_comps <- c(flowCore_comps, compensate(flowframes[[frame]], as.matrix(comp_matrix)))
        flowframes[[frame]] <- compensate(flowframes[[frame]], as.matrix(comp_matrix))
...
gate_population <- function(flow_frame, population, poplist, gatelist, gate_defs, lut) { #need to pass flow_frame (fcb flowCore file), gatelist (get_gates), population name (string), poplist (get_populations), lut
    ...
    for(j in pop.gates) { #for gate in pop_gates
        ...
        if (any(lut[channel.ind, "scaleType"] == 4)) {
            ...
            exprs(flow_frame)[,channel.char[ind]] <- asinh(exprs(flow_frame)[,channel.char[ind]]/lut[channel.ind[ind], "cofactor"])
        }
        
        #can now use rectangle gates!!!
        if(gate_defs[[i]][["type"]] == "RectangleGate") {
            gate.i <- rectangleGate(gate_defs[[i]][["coords"]])
            flow_frame <- gatein(flow_frame, gate.i)
        }
        else {
            gate.i <- polygonGate(gate_defs[[i]][["coords"]])
            flow_frame <- gatein(flow_frame, gate.i)
        }
        ...
        if (any(lut[channel.ind, "scaleType"] == 4)) {
            ind <- which(lut[channel.ind, "scaleType"] == 4)
            exprs(flow_frame)[,channel.char[ind]] <- sinh(exprs(flow_frame)[,channel.char[ind]])*lut[channel.ind[ind], "cofactor"]
        }
...
fcb_select <- function(input, output, session, setup){
    ...
    observe({
        ...
        if(!is.null(selected_fcb)) {
            channels <- unname(colnames(exprs(selected_fcb)))
    ...
    fcb_dfs <- eventReactive(input$df_button, {
        ...
        fcslog <- (
            list("popfcs" = description(pop_gated_flowFrame),
                 "bc1fcs" = tryCatch({description(pop_gated_flowFrame)}, error = NULL),
                 "bc2fcs" = tryCatch({description(pop_gated_flowFrame)}, error = NULL))
        )
        modulelog <- (
            list("comp" = comp,
                 "gate_defs" = gate_defs,
    ...
    output$cutoff_plot <- renderPlot({
        ...
        names(df) <- strRep(strRep(names(df), "-", '_'), " ", "_") #sanitize for ggplot
        yaxis <- strRep(strRep(input$db_bc2_channel, "-", '_'), " ", "_")
        xaxis <- strRep(strRep(input$db_bc1_channel, "-", '_'), " ", "_")
        ...
        ggplot(df, aes_string(x=xaxis , y = yaxis)) +
            geom_hex(bins = 200) +
            scale_y_continuous(trans= logicle_trans(), name = yaxis,
                               breaks = major.ticks,
                               labels = major.ticks,
                               minor_breaks = minor.ticks) +
            scale_x_continuous(trans= logicle_trans(), name = xaxis,
                               breaks = major.ticks,
                               labels = major.ticks,
                               minor_breaks = minor.ticks) +
            scale_fill_viridis(option = "magma", begin = 0, end = 1) +
            coord_cartesian(xlim = c(-1e2, 2e5), ylim = c(-1e2, 2e5)) +
            geom_vline(xintercept = bc1cut, col = "orange", size = 1) +
            geom_hline(yintercept = bc2cut, col = "blue", size = 1) +
            ggtitle("Removing non-barcoded cells")
